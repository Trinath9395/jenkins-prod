pipeline {
    ...
    stages {
        ...
        stage ('push') {
            steps {
                withCredentials([usernamePassword(credentialsId: 'dockerhub-auth', passwordVariable: 'DOCKERHUB_PWD', usernameVariable: 'DOCKERHUB_USER')]) {
...
stage('deploy') {
      steps {
        ...
        sh 'docker rm -f trinathr-flask || true'
        sh 'docker run -d --name trinathr-flask -p 5000:5000 "$IMAGE:$TAG"'
